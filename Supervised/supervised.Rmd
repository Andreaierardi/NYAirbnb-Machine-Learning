...
lm.fit <- lm(price~latitude+ longitude+ room_type+neighbourhood_group, data=train)
summary(lm.fit)
...
lm.fit <- lm(price~latitude+ longitude+ room_type, data=train_filtered[-1])
summary(lm.fit)
pr.lm <- predict(lm.fit,test)
MSE.lm <- sum((pr.lm - test$price)^2)/nrow(test)
...
lm.fit <- lm(price~latitude+ longitude, data=train_filtered[-1])
summary(lm.fit)
pr.lm <- predict(lm.fit,test)
MSE.lm <- sum((pr.lm - test$price)^2)/nrow(test)
...
train_filtered = train %>% filter(room_type == 1)
lm.fit <- lm(price~latitude+ longitude+ neighbourhood_group, data=train_filtered[-4])
summary(lm.fit)
pr.lm <- predict(lm.fit,test)
MSE.lm <- sum((pr.lm - test$price)^2)/nrow(test)
...
tree_reg =tree(price~ latitude+longitude+room_type, data = train_filtered)
summary(tree_reg)
...
cat("Neighboorhood group = Manhattan and room_type = Entire home/apt\n ")
train_filtered = train %>% filter(neighbourhood_group == 1 & room_type == 1)
tree_reg =tree(price~latitude+longitude, data = train)
summary(tree_reg)
...
cat("Neighboorhood group = Manhattan\n ")
train_filtered = train %>% filter(neighbourhood_group ==5)
