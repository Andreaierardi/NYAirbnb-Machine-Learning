...
# Data Inspection
```{r}
ok = ok %>%filter( price < 500)

ok = as.data.frame(scale(ok))
ggplot(as.data.frame(scale(ok)),aes(x=longitude,y=latitude, col=price))+geom_point()

filtered = ds%>%filter( price < 500)

ggplot(filtered,aes(x=longitude,y=latitude, col=price))+geom_point()


```

```{r}
ggplot(dataset,aes(x=longitude,y=latitude, col=price))+geom_point()
...
```
```{r}
sorted = sort(ds$price,decreasing = T) 
sorted[20000:20100]
```
...
clean_data = function(ds, pr, region)
{
  ds =ds[c(5,7:10)]
 
  ds=   ds%>%filter( price < pr)
  ds = ds %>% filter(neighbourhood_group == region)
  numerical = c("price")
...
dataset = clean_data(ds,500,"Manhattan")
...
```{r}
lm.fit <- lm(price~latitude+ longitude+room_type, data=train)
summary(lm.fit)
...
```
```{r}
without_out = clean_data(ds %>% filter( price < 1000) )
without_out
lm.fit <- lm(price~., data=without_out)
summary(lm.fit)
pr.lm <- predict(lm.fit,test)
MSE.lm <- sum((pr.lm - test$price)^2)/nrow(test)
...
par(mfrow=c(1,2))
plot(train$price,predict(rf2),col='red',main='OOB error',pch=18,cex=0.7)
abline(0,1,lwd=2)
legend('bottomright',legend='NN',pch=18,col='red', bty='n')
```
```{r}
